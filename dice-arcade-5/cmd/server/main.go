...
func (s *server) JoinGame(ctx context.Context, req *gamemanagerpb.JoinGameRequest) (*gamemanagerpb.JoinGameResponse, error) {
	...
	s.pub.Publish("player.joined", fmt.Sprintf("Player %s joined game %s", pid, id))
...
func (s *server) PlayTurn(ctx context.Context, req *gamemanagerpb.PlayTurnRequest) (*gamemanagerpb.PlayTurnResponse, error) {
	...
	if strings.Contains(outcome, "WIN") {
		s.pub.Publish("game.won", fmt.Sprintf("Player %s won in game %s", pid, id))
	}
...
func main() {
	...
	pub, err := mq.Connect("amqp://admin:password@localhost:5672/")
	if err != nil {
